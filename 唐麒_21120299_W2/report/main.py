...
def cluster_result_show(image_name, image):
    ...
    cv2.imwrite(image_name, image_save)
    plt.imshow(image, cmap='gray')
    plt.show()
...
mean_glcm = (calculate_glcm(input_win=windowed_image[w][h], step_x=1, step_y=0, gray_level=gray_level) +
            ...
            calculate_glcm(input_win=windowed_image[w][h], step_x=-1, step_y=1,gray_level=gray_level)) / 4
...
def extract_texture_representation(win_size, image, stride=(1, 1), gray_level=16, feat_keys=[]):
    ...
    homogeneity = glcm_features_calc.calculate_glcm_homogeneity()
    ...
    feat_dict = {"Mean": mean, "Variance": variance, "Contrast": contrast, "Dissimilarity": dissimilarity,
                 ...
                 "Homogeneity": homogeneity, "Autocorrelation": auto_correlation}
...
def main():
    ...
    glcm_statistics = {
        "Texture_mosaic_1": ["Mean", "Variance", "Entropy", "Energy", "Inertia", "Correlation", "Autocorrelation"],
        "Texture_mosaic_2": ["Mean", "Variance", "Entropy", "Energy", "Correlation", "Inertia", "Homogeneity"],
    ...
    image_name = "Texture_mosaic_2.jpg"
    ...
    glcm_texture_feats = extract_texture_representation(win_size=19, image=image, stride=(1, 1), gray_level=16,
