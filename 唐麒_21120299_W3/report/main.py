...
def correlation_match(template, scene):
    ...
    scene_slides = np.lib.stride_tricks.as_strided(scene, shape=(new_h, new_w, template_h, template_w),strides=strides)
...
def hausdorff_match(idx, template, scene):
    ...
    for h in range(new_h):
        for w in range(new_w):
            ...
            distance[h][w] = np.max([calculate_hausdorff_distance(template_x, win_x), calculate_hausdorff_distance(template_y, win_y)])
...
def main():
    ...
    start = time.time()
    localize([template_1, template_2], scene, correlation_match, "ncc")
