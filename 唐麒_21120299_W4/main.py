...
def main():
    ...
    image_with_rect = cv2.rectangle(image, (location[0],location[1]), (location[0]+location[2],location[1]+location[3]), (0, 0, 255), 2)
    cv2.imwrite("./save/" + str(1) + ".png", image_with_rect)
    ...
    for pic_i in range(2, 101):
        ...
        cv2.imwrite("./save/"+str(pic_i)+".png",image_with_rect)
        cv2.imshow('Car Tracker', image_with_rect)
        k = cv2.waitKey(60) & 0xff
        if k == 27:
            break
    print("单张耗时:", total_time/100, "每秒处理:", 100 / total_time)
