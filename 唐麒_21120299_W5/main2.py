...
# 读取图片集合，返回RGB图片
def load_data_set(path, is_training=True):
    if is_training:
        return load_train_dataset(path)
    else:
        return load_test_dataset(path)


# gmm模型初始化
def gmm_models_create():
    # 读取首张图片帧
    first_image = cv2.imread('./Scene_Data/0000.jpg')
...
if __name__ == '__main__':
    ...
    gmm_model_path = './models_learned/gmm_models_maxK={0}_alpha={1}_T={2}_sigma={3}.pkl'.format(models[0].k,
                                                                                                 models[0].alpha,
                                                                                                 models[0].t, SIGMA)
    if not os.path.exists('./models_learned/'):
        os.mkdir('./models_learned/')
    # 如果模型已存在则直接加载
    if not os.path.exists(gmm_model_path):
        # 加载训练集
        images = load_data_set('./Scene_Data/', is_training=True)
    ...
    images = load_data_set('./Scene_Data/', is_training=False)
    # 实时显示剪除效果
    param_str = 'maxK={0} alpha={1} T={2} SIGMA={3}'.format(models[0].k, models[0].alpha, models[0].t, SIGMA)
    # plt.ion()

    result_save_path = "./results/multi_channel_maxK={0}_alpha={1}_T={2}_sigma={3}/".format(models[0].k, models[0].alpha,
                                                                                            models[0].t, SIGMA)
    ...
    for i in range(len(images)):
        ...
        cv2.imwrite(result_save_path + str(i+109) + ".png", frame_optimized)
