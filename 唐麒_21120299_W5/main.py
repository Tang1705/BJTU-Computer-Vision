...
if __name__ == '__main__':
    ...
    if not os.path.exists(gmm_model_path):
        # 加载训练集
        frames = load_data_set('./Scene_Data/', is_training=True)
    ...
    frames = load_data_set('./Scene_Data/', is_training=False)
    # 实时显示剪除效果
    param_str = 'maxK={0} alpha={1} T={2} SIGMA={3}'.format(model.k, model.alpha, model.t, SIGMA)
    # plt.ion()

    result_save_path = "./results/maxK={0}_alpha={1}_T={2}_sigma={3}/".format(model.k, model.alpha, model.t, SIGMA)
